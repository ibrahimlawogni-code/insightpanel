...
- **TeamVallee (site web) :** SPA dédiée au suivi de performance de l'équipe Vallée, hébergée sur GitHub Pages (https://ibrahimlawogni-code.github.io/insightpanel/team-vallee.html), fichier source : livrables/sites-web/TeamVallee.html, backend GAS séparé (VALLEE_URL). Fonctionnalités actives : authentification par rôle (superviseur vs RA/admin), dashboard avec KPIs (GA, MoMo, DFA Actifs, Stock SIM), sélecteur de période (jour/semaine/mois/trimestre/année/période), saisie du jour, suivi stocks SIM, suivi dysfonctionnements (avec localité libre ou "Toutes les localités"), paramètres objectifs mensuels par mois avec calcul automatique MoMo = GA × 90% et répartition DFA Actifs par superviseur, restauration de la vue active après refresh. Cibles individuelles superviseurs calculées proportionnellement à leurs DFA Actifs (pas de division égale). Hook Git pre-commit synchronise TeamVallee.html vers team-vallee.html à la racine.
